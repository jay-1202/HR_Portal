  ...
  const handleLogin = async(e) => {
    ...
    if (input.username.length !== 0 && input.password.length !== 0) {
      ...
      if (input.password.includes(input.username)) {
        setInputError({
          reason: "Password contains username",
          boolean: true,
        });
      } else {
        ...
        const response = await fetch(API_URL + "/api/auth/signup" , {
          ...
          body:JSON.stringify({
            Email_id: input.username, Password: input.password, Firstname: input.firstname, Lastname: input.lastname
          })
        })
    ...
    if (input.username.length === 0 || input.password.length === 0) {
      setInputError({
        reason: "Username or Password is Empty",
        boolean: true,
      });
    }
  }
    ...
    <div style={{ display: "flex", justifyContent: "center" }}>
      <Card sx={{ margin: 7, padding: 5 }}>
        ...
        <CardActions>
          ...
          >
            <Button variant="contained" onClick={handleLogin} sx={{marginBottom: '10px'}}>
              Signup
            </Button>
          </div>
        </CardActions>
